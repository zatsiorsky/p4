...
class Learner(object):
	...
	def choose_action(self, state):
		...
		epsilon = self.epsilon * np.exp(-self.n_iters / float(500))
...
if __name__ == '__main__':
	...
	for x in range(250,1000):
		...
		df.to_csv('csvs_final/' + name + str(x) + ".csv", index=0)
		...
		with open("grid_results_final.csv", "a") as myfile:
			myfile.write(','.join(map(str,combined)) + "\n")

		# shout at command line
		elapsed = datetime.datetime.now() - now
		print "Elapsed 100s : {},  Elapsed time: {}".format(x+1, str(elapsed))
